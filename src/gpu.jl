...
# container type for all GPU variables
#type PlinkGPUVariables{T <: Float}
#    df_buff     :: cl.Buffer{T}
...
function PlinkGPUVariables{T <: Float}(
    ...
    x      :: BEDFile{T},
    y      :: DenseVector{T},
#    kern   :: ASCIIString      = readall(open(ENV["HOME"] * "/.julia/v0.4/PLINK/src/kernels/iht_kernels64.cl")),
    ...
    wg_size     = 512
    y_chunks    = div(x.geno.n, wg_size) + (x.geno.n % wg_size != 0 ? 1 : 0)
...
#    program     = cl.Program(ctx, source=kern) |> cl.build!
    ...
    red_buff    = cl.Buffer(T,    ctx, (:rw), x.geno.p + x.covar.p*y_chunks) :: cl.Buffer{T}
...
#    PlinkGPUVariables(df_buff, x_buff, y_buff, mask_n, mask_buff, queue, m_buff, p_buff, red_buff, xtyk, rxtyk, reset_x, wg_size, y_chunks, r_chunks, x.geno.n, x.geno.p, x.covar.p, n32, p32, y_chunks32, blocksize32, wg_size32, y_blocks32, r_length32, genofloat)
...
#    @inbounds for case = 1:x.geno.n
    for case = 1:x.geno.n
        if mask_n[case] == 1
            df[x.geno.p+snp] += y[case] * (x.covar.x[case,snp] - m) * s
        end
    end
    return nothing
end
...
copy_y!{T <: Float}(v::PlinkGPUVariables{T}, y::SharedVector{T}) = cl.wait(cl.copy!(v.queue, v.y_buff, sdata(y)))
reset_x!{T <: Float}(v::PlinkGPUVariables{T}) = cl.wait(cl.call(v.queue, v.reset_x, (v.wg_size*v.r_chunks,1,1), nothing, v.red_buff, v.r_length32, zero(T)))
xty!{T <: Float}(v::PlinkGPUVariables{T}) = cl.wait(cl.call(v.queue, v.xtyk, (v.wg_size*v.y_chunks, v.p, 1), (v.wg_size, 1, 1), v.n32, v.p32, v.y_chunks32, v.blocksize32, v.wg_size32, v.x_buff, v.red_buff, v.y_buff, v.m_buff, v.p_buff, v.mask_buff, v.genofloat))
xty_reduce!{T <: Float}(v::PlinkGPUVariables{T}) = cl.wait(cl.call(v.queue, v.rxtyk, (v.wg_size,v.p,1), (v.wg_size,1,1), v.n32, v.y_chunks32, v.y_blocks32, v.wg_size32, v.red_buff, v.df_buff, v.genofloat))
copy_xty!{T <: Float}(v::PlinkGPUVariables{T}) = cl.wait(cl.copy!(v.queue, sdata(xty), v.df_buff))
function Base.At_mul_B!{T <: Float}(
    xty    :: SharedVector{T},
    x      :: BEDFile{T},
    y      :: SharedVector{T},
...
#@show sum(xty[1:x.geno.p])
#@show sum(y)
#@show sum(cl.read(v.queue, v.x_buff))
#@show sumabs2(cl.read(v.queue, v.m_buff))
#@show sumabs2(cl.read(v.queue, v.p_buff))
#@show sumabs2(cl.read(v.queue, v.mask_buff))
    cl.wait(cl.copy!(v.queue, v.y_buff, sdata(y)))
#@show sum(cl.read(v.queue, v.y_buff))
#@show sum(cl.read(v.queue, v.red_buff))
#   cl.fill!(queue, red_buff, zero(T))    # only works with OpenCL 1.2
...
#@show sum(xty[1:x.geno.p])
#    @inbounds for snp = 1:v.p2
#        df_x2!(snp, xty, x, y, mask_n)
#    end
